...
def create_vector_embeddings():
    if "vectors" not in st.session_state:
        ...
        st.session_state.loader = PyPDFDirectoryLoader("RAG Document Q&A with GROG & LLAMA3/research_papers")
        ...
        for attempt in range(retries):
            try:
                ...
                if isinstance(e, httpx.HTTPStatusError) and e.response.status_code == 429:  # Rate limit error
                    if attempt < retries - 1:
                        st.write("Rate limit reached, retrying...")
